...
class PlaywrightComputer(Computer):
    ...
    def __enter__(self):
        ...
        if debug_port:
            ...
            try:
                self._browser = self._playwright.chromium.connect_over_cdp(f"http://localhost:{debug_port}")
                # Use the first context if available, or create new?
                # Usually connect_over_cdp gives a browser where we can get contexts.
                # But we want to control the EXISTING page if possible, or just a new tab in that window.
                if self._browser.contexts:
                    self._context = self._browser.contexts[0]
                else:
                    self._context = self._browser.new_context(viewport=None) # Don't resize user's window
        ...
        # If we connected to existing Chrome, we might want to use the active page
        if debug_port and self._context.pages:
